#!/usr/bin/env python3
"""step4_urdf_fix_inertia.py — Patch cable-manipulator URDF with realistic inertial values.

Onshape-to-robot assigns masses from solid CAD bodies (default material density),
leading to large overestimates:

  pulley_htd_5m_60t   →  5.31 kg   WRONG  (full assembly volume × steel density)
  link2_tendon        →  0.52 kg   OK     (mostly aluminium + small parts)
  base_mate           →  0.90 kg   OK     (fixed base — no effect on joint torques)

This script replaces each <inertial> block with values derived from:
  • Realistic mass estimates (motor 0.40 kg + gearbox 0.45 kg + Al arm 0.35 kg + misc)
  • COM placed at geometric midpoint of each arm (x = L/2, y = z = 0)
  • Diagonal inertia tensor computed from uniform-rod + concentrated-mass model
  • Off-diagonal elements set to zero (clean; cross-products ≈ 0 for symmetric arm)

Geometry reference (from URDF joint origins):
...
-----
  ...
  python step4_urdf_fix_inertia.py --dry-run

  # Patch in-place (auto-backup to .back unless one already exists):
  python step4_urdf_fix_inertia.py

  # Patch to a different output file:
  python step4_urdf_fix_inertia.py --out patched.urdf

  # Restore original from auto-backup:
  python step4_urdf_fix_inertia.py --restore

  # Custom URDF path:
  python step4_urdf_fix_inertia.py --urdf path/to/robot.urdf
"""
...
# ─────────────────────────────────────────────────────────────────────────────
DEFAULT_URDF = Path(
    "model_using_onshape_to_robot/manipulator_cable/manipulator_cable_obj.urdf"
)

# ─────────────────────────────────────────────────────────────────────────────
# Arm geometry  (from URDF joint origin x-components, which give the in-plane
# reach for a SCARA with Z-axis revolute joints)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
#
# 'realistic'  — best estimate for the actual robot (motor + gearbox + Al arm)
# 'light'      — lightweight build (carbon-fibre arm, smaller motor)
# 'heavy'      — conservative upper-bound (steel arm, larger motor/gearbox)
#
...
PRESETS: dict[str, dict] = {
    ...
    # ── realistic (default) ──────────────────────────────────────────────────
    # Motor 0.40 kg + Gearbox 0.45 kg + Al arm 0.35 kg + pulleys/misc 0.30 kg
    "realistic": {
        "link1_mass":  1.50,   # kg  (vs. Onshape 5.31 — 3.5× over-estimate)
        "link2_mass":  0.50,   # kg  (vs. Onshape 0.52 — already reasonable)
        "link1_com_x": L1 / 2.0,   # geometric midpoint of arm reach  [m]
        "link2_com_x": L2 / 2.0,
        # proximal cluster (motor + gearbox + pulleys) near Joint-1
        "m1_proximal": 1.15,   # kg — motor + gearbox + pulleys at base
        "r1_proximal": abs(L1 / 2.0 - 0.08),  # distance of cluster from COM [m]
        "m2_tip":      0.15,   # kg — cup + retained liquid at EE tip
    },

    # ── light (carbon-fibre / small motor) ───────────────────────────────────
    # Motor 0.20 kg + Gearbox 0.25 kg + CF arm 0.15 kg + misc 0.15 kg
    "light": {
        "link1_mass":  0.75,
        "link2_mass":  0.30,
        "link1_com_x": L1 / 2.0,
        "link2_com_x": L2 / 2.0,
        "m1_proximal": 0.60,
        "r1_proximal": abs(L1 / 2.0 - 0.07),
        "m2_tip":      0.10,
    },

    # ── heavy (steel arm / large servo) ──────────────────────────────────────
    # Motor 0.70 kg + Gearbox 0.80 kg + Steel arm 0.80 kg + misc 0.50 kg
    "heavy": {
        "link1_mass":  2.80,
        "link2_mass":  0.80,
        "link1_com_x": L1 / 2.0,
...
def patch_text(text: str, patches: dict, dry_run: bool = False) -> tuple[str, list[dict]]:
    ...
    The replacement is scoped to each link's own text range so that a link
    appearing before another is never accidentally modified.
    ...
    for link_name, p in patches.items():
        ...
        if link_start == -1:
            print(f"  ⚠  Link '{link_name}' not found — skipped.")
            continue

        # Find the matching </link> (first occurrence after the opening tag)
        link_end_tag = "</link>"
        link_end = text.find(link_end_tag, link_start)
        if link_end == -1:
            print(f"  ⚠  Missing </link> for '{link_name}' — skipped.")
            continue
        link_end += len(link_end_tag)
        link_block = text[link_start:link_end]

        # ── Collect 'before' stats via ET (won't blow up on existing URDF quirks)
        try:
        ...
        # ── Detect indentation actually used in the block for the <inertial> tag
        m = _INERTIAL_RE.search(link_block)
        indent = "    "                     # fallback
        if m:
            # Walk backwards from match start to find line indentation
...
def _print_report(rows: list[dict], kp_values: list[float], tau_max: float) -> None:
    ...
    # Parallel-axis theorem from each link's COM to its parent joint:
    #   M_joint_i += Izz_link + m_link * d²   where d = COM x-distance from joint
    ...
    # After-patch values (from report rows)
    ...
    # Before-patch values (original URDF, from report rows)
    ...
    # Original COMs from URDF (x-components hardcoded from the .back file)
    cx1o, cx2o = 0.0456, 0.1092
    ...
    for Kp in kp_values:
        ...
        print(f"  τ1 for 5° error (Kp={Kp:.0f}):  {t_old:.2f} → {t_new:.2f} Nm"
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "  realistic — motor+gearbox+Al arm  L1=1.50 kg  L2=0.50 kg\n"
            "  light     — CF arm + small motor  L1=0.75 kg  L2=0.30 kg\n"
            "  heavy     — steel arm + large servo L1=2.80 kg  L2=0.80 kg"
        ),
    )
    ...
    ap.add_argument(
        ...
        default=[100.0, 400.0],
        metavar="KP",
        help="Kp gain(s) for the τ-saturation report  (default: 100 400)",
    )
    ap.add_argument(
        "--tau-max",
        type=float,
    ...
    # ── Backup (only if writing in-place and no backup exists yet) ────────────
    ...
    if out_path == urdf_path and not back_path.exists():
        ...
        print(f"  ℹ  Backup already exists at {back_path.name} — not overwriting.")
