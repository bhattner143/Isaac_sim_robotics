...
parser.add_argument('--target-x', type=float, default=-1.0, help='Target X position [m]')
parser.add_argument('--target-y', type=float, default=1, help='Target Y position [m]')
parser.add_argument('--cart-x-init', type=float, default=-1.5, help='Initial cart X position [m] (default: use manipulator EE position)')
parser.add_argument('--cart-y-init', type=float, default=1.0, help='Initial cart Y position [m] (default: use manipulator EE position)')
...
# CONSTANTS
# ============================================================================
# End-effector offset from link2 frame to cup center (from URDF)
...
class CupManipulator(RobotBase):
    ...
    # --- Cup-center EE pose relative to link2, from your URDF small_ball (cup middle) origin ---
    # URDF: xyz="-1.2545 0 -0.188125" rpy="3.14159 0 0"
    EE_XYZ_LINK2 = np.array([-1.2545, 0.0, -0.188125])   # meters
    EE_RPY_LINK2 = np.array([np.pi, 0.0, 0.0])           # radians
    ...
    def get_end_effector_position(self, plant, context):
    ...
    def set_state_in_plant(self, plant: MultibodyPlant, context, user_state: np.ndarray):
        ...
        drake_positions = np.array([q2, q1])
        drake_velocities = np.array([q2_dot, q1_dot])
        plant.SetPositions(context, self.model_instance, drake_positions)
        plant.SetVelocities(context, self.model_instance, drake_velocities)

    def get_positions_user_order(self, plant: MultibodyPlant, context) -> np.ndarray:
        drake_positions = plant.GetPositions(context, self.model_instance)
        return np.array([drake_positions[1], drake_positions[0]])
...
def check_trajectory_feasibility(manipulator, plant, trajectory_points, q_init=None):
    ...
    def forward_kinematics(q):
        ...
        for i, joint_index in enumerate(manip_joint_indices):
            ...
            if joint.num_velocities() > 0 and i < 2:
                # Drake's joint ordering is [link2_link1, link1_base] = [q2, q1]
                # Input q is [q1, q2], so we need [q[1], q[0]]
                joint.set_angle(context, [q[1], q[0]][i])
...
class ComputedTorqueEEController(LeafSystem):
    ...
    def calc_torques(self, context, output):
        ...
        manip_joint_indices = self.plant.GetJointIndices(self.manipulator.model_instance)
...
class ComputedTorqueJointSpaceController(LeafSystem):
    ...
    def calc_torques(self, context, output):
        ...
        # Extract torques in natural order [τ1, τ2]
        tau = np.array([tau_all[idx] for idx in manip_velocity_indices])
        tau = np.clip(tau, -self.tau_max, self.tau_max)
...
class ManipulatorStateConverter(LeafSystem):
    """
    Converts manipulator state from Drake ordering [q2, q1, q̇2, q̇1] 
    to natural ordering [q1, q2, q̇1, q̇2].
    
    Input:
      0: drake_state (4) = [q2, q1, q̇2, q̇1]  (from plant.get_state_output_port)
    Output:
      0: natural_state (4) = [q1, q2, q̇1, q̇2]
    """
    
    def __init__(self):
        LeafSystem.__init__(self)
        self.DeclareVectorInputPort("drake_state", 4)
        self.DeclareVectorOutputPort("natural_state", 4, self.convert_state)
    
    def convert_state(self, context, output):
        drake_state = self.get_input_port(0).Eval(context)
        q2, q1, q2_dot, q1_dot = drake_state
        # Convert to natural [q1, q2, q̇1, q̇2]
        natural_state = np.array([q1, q2, q1_dot, q2_dot])
        output.SetFromVector(natural_state)
...
class ManipulatorIKDesiredAngles(LeafSystem):
    ...
    def calc_desired_angles(self, context, output):
        cart_state = self.get_input_port(0).Eval(context)
        x_target, y_target = cart_state[0], cart_state[1]
        x_dot, y_dot = cart_state[2], cart_state[3]
        
        # Get manipulator joint indices (needed for extraction)
        manip_joint_indices = list(self.plant.GetJointIndices(self.manipulator.model_instance))
        
        # Both cart and manipulator work in X-Y plane after base rotation
        # Direct mapping: cart [X, Y] → manipulator target [X, Y]
        # Solve IK using previous solution as warm start
        q_desired, success = solve_initial_pose_via_ik(
            self.plant, self.manipulator, 
            np.array([x_target, y_target]),  # Will be mapped to [X, Z] in IK solver
            self.q_prev, 
        ...
        J_xy = J_full[[0, 1], :][:, manip_velocity_indices]  # 2x2: rows [X, Y], manip columns
        
        # Compute desired joint velocities: q̇_d = J^+ * ẋ_cart
        # Direct mapping: cart velocity [ẋ, ẏ] = manipulator [ẋ, ẏ]
        J_pinv = np.linalg.pinv(J_xy)
        q_dot_desired = J_pinv @ np.array([x_dot, y_dot])  # Already natural order from URDF
...
def solve_initial_pose_via_ik(
    ...
    """
    Solve for joint angles [q1, q2] (USER order) that place the EE at target (x, y).
    Keeps the EE z coordinate near the forward-kinematics value of the seed config.

    Assumptions:
      - manipulator.set_positions_user_order expects [q1, q2]
      - manipulator.get_positions_user_order returns [q1, q2]
      - An EE frame exists (recommended) named manipulator.ee_frame_name (or ee_frame_name).
        If not, you can still constrain a point on link2, but then you must use the correct
        offset point expressed in link2.
    """
    ...
    manipulator.set_positions_user_order(plant, ik_context, q_seed)

    world = plant.world_frame()

    # Prefer a named EE frame you added (e.g., "ee" or "cup_center")
    if ee_frame_name is None:
        ee_frame_name = getattr(manipulator, "ee_frame_name", None) or getattr(manipulator, "EE_FRAME_NAME", None)

    if ee_frame_name is not None:
        ee_frame = plant.GetFrameByName(ee_frame_name, manipulator.model_instance)
        # constrain the ORIGIN of the EE frame
        p_BQ = np.zeros(3)
    else:
        # Fallback (not recommended): constrain a point on link2 body frame using an offset
        link2_body = plant.GetBodyByName("link2", manipulator.model_instance)
        ee_frame = link2_body.body_frame()
        # Use the correct offset expressed in link2 (update to match your URDF!)
        p_BQ = np.asarray(manipulator.EE_XYZ_LINK2).reshape(3,)

    # Compute seed EE position (to pick z)
    ...
    z_fixed = ee_pos_seed[2]

    if verbose:
        print(f"  Seed EE position: ({ee_pos_seed[0]:.3f}, {ee_pos_seed[1]:.3f}, {ee_pos_seed[2]:.3f})")
        print(f"  Target: ({target_xy[0]:.3f}, {target_xy[1]:.3f}, {z_fixed:.3f})")
        print(f"  Tolerance: ±{pos_tol:.6f} m")

    # Position constraint in world
    lower = np.array([target_xy[0], target_xy[1], z_fixed]) - pos_tol
    upper = np.array([target_xy[0], target_xy[1], z_fixed]) + pos_tol
    ...
    # Add cost: stay near seed configuration
    # The IK's q() returns all generalized positions in the plant
    # We need to set initial guess and cost for the entire plant state
    q0_all = plant.GetPositions(ik_context)  # All positions in the plant
    
    prog.AddQuadraticErrorCost(np.eye(len(q0_all)), q0_all, q_vars)
    ...
    # Extract the solution and apply it to the IK context to read back manipulator positions
    ...
    q_sol_drake = plant.GetPositions(temp_context, manipulator.model_instance)
...
def run_lqr_manip_ee_traj_track(
    ...
    # Calculate manipulator EE position and velocity over time from actual joint states
    ee_positions = []
    ee_velocities = []
    for i in range(len(t)):
        # Create a context for this timestep
        temp_context = plant.CreateDefaultContext()
        # Set manipulator state at this timestep (natural [q1, q2, q̇1, q̇2] order)
        manipulator.set_state_in_plant(plant, temp_context, manip_state_data[0:4, i])
        
        # Calculate EE position
        ee_pos = manipulator.CalcPosition(plant, temp_context)
        ee_positions.append(ee_pos[:2])  # Only x, y
        ...
        ee_vel = J_xy @ manip_state_data[2:4, i]
        ee_velocities.append(ee_vel)
    
    ee_positions = np.array(ee_positions).T
    ee_velocities = np.array(ee_velocities).T
...
def run_lqr_manip_ik_track(*args, **kwargs):
    """TODO: Implement LQR with manipulator IK tracking."""
    print(colored("⚠️ This mode is not yet implemented!", "yellow"))
    print(colored("   Please use 'manip-ik-follows-cart' or 'finite-horizon-lqr-for-min-effort_cart_pend_only' instead.", "yellow"))
    sys.exit(1)


def run_manip_ik_follows_cart(*args, **kwargs):
    """TODO: Implement manipulator IK follows cart mode."""
    print(colored("⚠️ This mode is not yet implemented!", "yellow"))
    print(colored("   Please use 'finite-horizon-lqr-for-min-effort_cart_pend_only' instead.", "yellow"))
    sys.exit(1)
...
def plot_frames_top_view(plant, context, manipulator, cart_model, title="Frame Orientation (Top View)"):
    ...
    # Link2 (EE) frame
    link2_frame = plant.GetFrameByName("link2", manipulator.model_instance)
    ...
    ee_pos = plant.CalcPointsPositions(
        context, link2_frame, MANIPULATOR_EE_OFFSET, plant.world_frame()
    ).flatten()
    # Draw EE frame (same rotation as link2, but at offset position)
    draw_frame_2d(ax1, ee_pos[[0, 1]], link2_rot, scale=0.25,
...
def main():
    ...
    if args.mode == 'scene-viz':
        ...
        # These allow us to apply torques in modes like 'manip-ik-follows-cart'
        for joint_index in plant.GetJointIndices(manipulator.model_instance):
            joint = plant.get_joint(joint_index)
            # Only add actuator if joint has degrees of freedom
            if joint.num_velocities() > 0:
                plant.AddJointActuator(f"tau_{joint.name()}", joint)
        
        # Add end-effector frame BEFORE finalization (required for IK)
        ...
        for joint_idx in plant.GetJointIndices(manipulator.model_instance):
            joint = plant.get_mutable_joint(joint_idx)
            if joint.num_velocities() > 0:
                joint.set_default_damping_vector([1000.0])  # High damping to lock joint
        ...
        # Get EE world frame position directly
        ee_frame = plant.GetFrameByName("link2", manipulator.model_instance)
        ee_world_pos = plant.CalcPointsPositions(
            temp_context, ee_frame, MANIPULATOR_EE_OFFSET, plant.world_frame()
        ).flatten()
                        ...
                        if len(parts) == 3 and parts[0] in ['c', 'e']:
                            ...
                            if command == 'c':
                                ...
                                q_solution, success = solve_initial_pose_via_ik(
                                    plant, manipulator, target_xy, current_manip_q, pos_tol=0.05, verbose=True
                                )
                                ...
                                if success:
                                    ...
                                    ee_frame = plant.GetFrameByName("link2", manipulator.model_instance)
                                    ee_actual = plant.CalcPointsPositions(
                                        plant_context, ee_frame, MANIPULATOR_EE_OFFSET, plant.world_frame()
                                    ).flatten()
        ...
        # Add end-effector frame BEFORE finalization (required for EE tracking)
        manipulator.add_end_effector_frame(plant)
        print(colored(f"✓ End-effector frame '{manipulator.EE_FRAME_NAME}' added to manipulator", "green"))
        ...
        # ========================================================================
        # Initial configuration in natural [q1, q2] order
        initial_q = np.array([np.deg2rad(0.0), np.deg2rad(0.0)])  # [q1, q2]
        ...
        # Both manipulator and cart work in X-Y plane after rotation: direct mapping [X, Y]
        ee_pos_3d = ee_pos_init  # Keep full 3D coordinates  # [manip_X, manip_Y, 0]
        
        # Define cart initial position at manipulator EE
        cart_init_pos = np.array([ee_pos_3d[0], ee_pos_3d[1], 0.0, 0.0])  # [x from EE_X, y from EE_Z, α, β]  # [x, y, α, β]
        ...
        X_WE = plant.CalcRelativeTransform(temp_context, plant.world_frame(), ee_frame)
        ee_world_pos = X_WE.translation()
        ...
        # Create fresh builder for control diagram
        control_builder = DiagramBuilder()
        control_builder.AddSystem(plant)
        control_builder.AddSystem(scene_graph)
        ...
        run_lqr_manip_ee_traj_track(
            control_builder, plant, scene_graph, meshcat, cart_model, manipulator,
            ee_pos_init, physics_config, impedance_config, zft_config, muscle_config, args
        )
    
    # elif args.mode == 'lqr-manip-ik-track':
    #     print(colored("\n🚀 Running LQR with manipulator IK tracking...", "cyan"))
    #     run_lqr_manip_ik_track(
    #         builder, plant, scene_graph, meshcat, cart_model, manipulator,
    #         ee_pos_init, physics_config, impedance_config, zft_config, muscle_config, args,
    #         cart_x_init=args.cart_x_init, cart_y_init=args.cart_y_init
    #     )
    
    # elif args.mode == 'manip-ik-follows-cart':
    #     print(colored("\n🚀 Running manipulator IK follows cart (no torque control)...", "cyan"))
    #     run_manip_ik_follows_cart(
    #         builder, plant, scene_graph, meshcat, cart_model, manipulator,
    #         ee_pos_init, physics_config, impedance_config, zft_config, muscle_config, args,
    #         cart_x_init=args.cart_x_init, cart_y_init=args.cart_y_init
    #     )
