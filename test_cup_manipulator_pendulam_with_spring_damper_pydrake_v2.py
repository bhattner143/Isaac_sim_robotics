...
# ── 5. Tendon spring (Series Elastic Actuator) ────────────────────────────────
_tendon = parser.add_argument_group('tendon spring SEA  [pd-control | computed-torque]')
_tendon.add_argument(
    '--tendon-stiffness', type=float, default=0.00,  # 0.05 N/m is very soft, 5000+ N/m is nearly rigid
    metavar='K_S',
    help='Cable spring stiffness [N/m].  '
         'Inserts a real spring between motor pulley and joint 2: '
         'F = k_s * r_p * (θ_m - q₂).  '
         'Weak spring (small k_s, e.g. 50-200 N/m) makes joint 2 sluggish/oscillatory.  '
         'Stiff spring (large k_s, e.g. 5000+ N/m) is nearly rigid.  '
         '0 = perfectly rigid tendon, SEA disabled.  (default: 0)',
)
_tendon.add_argument(
    '--tendon-damping', type=float, default=2.0,
    metavar='B_S',
    help='Cable viscous damping [N·s/m] — damps spring oscillations.  '
         '(default: 2.0)',
)
_tendon.add_argument(
    '--tendon-motor-inertia', type=float, default=5e-4,
    metavar='I_M',
    help='Motor rotor inertia reflected to joint-2 shaft [kg·m²].  '
         'Larger inertia → slower motor response → more lag.  '
         '(default: 5e-4)',
)

# ── 6. Computed-torque gains ─────────────────────────────────────────────────
...
class TendonSpringCompliance(LeafSystem):
    """Series Elastic Actuator (SEA) model for the joint-2 cable tendon.

    Physics
    ~~~~~~~
    A physical spring sits in the cable between the motor pulley and joint 2.
    The motor drives one end; the spring stretches and then pulls the joint:

        Cable stretch:   δ = r_p · (θ_m − q₂)                          [m]
        Spring force:    F_s = k_s · δ + b_s · δ̇                        [N]
        Torque on joint: τ₂ = F_s · r_p  =  k_s·r_p²·(θ_m−q₂) + b_s·r_p²·(θ̇_m−q̇₂)  [Nm]

    Motor equation of motion (Newton's 2nd law on motor shaft):

        I_m · θ̈_m = τ₂_cmd − F_s · r_p
                   = τ₂_cmd − k_s·r_p²·(θ_m−q₂) − b_s·r_p²·(θ̇_m−q̇₂)

    A weak spring (small k_s) → large stretch under load → joint 2 lags.
    A stiff spring (large k_s) → rigid-like, no meaningful lag.
    Joint 1 (direct-drive) passes τ₁ straight through, unchanged.

    States
    ~~~~~~
    x[0] = θ_m    — motor angle          [rad]
    x[1] = θ̇_m   — motor velocity       [rad/s]

    Ports
    ~~~~~
    Input:  ``torque_cmd``   [2]     [τ₁_cmd, τ₂_motor_cmd] from controller [Nm]
    Input:  ``plant_state``  [n]     plant state to read q₂, q̇₂
    Output: ``torque_out``   [2]     [τ₁, τ₂_spring] → plant actuator input [Nm]
    Output: ``spring_force`` [1]     cable spring force F_s [N]  (for logging)
    """
    ...
    def __init__(
        ...
        plant,
        manipulator,
        k_s: float = 500.0,   # cable spring stiffness  [N/m]
        b_s: float = 2.0,     # cable viscous damping   [N·s/m]
        I_m: float = 5e-4,    # motor inertia           [kg·m²]
    ) -> None:
        super().__init__()
        self._plant  = plant
        ...
        # Private plant context for reading q₂, q̇₂
        ...
        # Continuous state: [θ_m, θ̇_m]
        self.DeclareContinuousState(2)

    def _read_joint2(self, context):
        """Return (q₂, q̇₂) from current plant state."""
    ...
    def _spring_force(self, context):
        """Cable spring force F_s [N] = k_s·δ + b_s·δ̇."""
        x         = context.get_continuous_state_vector().CopyToVector()
        theta_m   = x[0];  theta_m_dot = x[1]
        q2, q2dot = self._read_joint2(context)
        r_p       = self._r_p
        delta     = r_p * (theta_m - q2)           # cable stretch [m]
        delta_dot = r_p * (theta_m_dot - q2dot)    # stretch rate  [m/s]
    ...
    def DoCalcTimeDerivatives(self, context, derivatives):
        ...
        F_s = self._spring_force(context)          # spring pulls back on motor [N]
        # Motor EOM: I_m·θ̈_m = τ₂_cmd − F_s·r_p
    ...
    def _calc_output(self, context, output):
        tau_cmd = self._cmd_port.Eval(context)
        # Torque delivered to joint 2 = spring force × r_p
        tau2_spring = self._spring_force(context) * self._r_p   # [Nm]
        output.SetFromVector(np.array([tau_cmd[0], tau2_spring]))

    def _calc_spring_force(self, context, output):
        output.SetFromVector(np.array([self._spring_force(context)]))

    def init_motor_angle(self, diagram_context, q2_init: float) -> None:
        """Set θ_m = q₂_init so there is no spring stretch at t=0."""
        ctx = self.GetMyMutableContextFromRoot(diagram_context)
        ctx.get_mutable_continuous_state_vector().SetAtIndex(0, q2_init)
        ctx.get_mutable_continuous_state_vector().SetAtIndex(1, 0.0)
...
class Simulation(ABC):
    ...
    def __init__(self, manip_config, meshcat=None,
                 tendon_stiffness: float = 0.0,
                 tendon_damping:   float = 2.0,
                 tendon_motor_inertia: float = 5e-4):
        self.manip_config          = manip_config
        self.meshcat               = meshcat
        self.tendon_stiffness      = float(tendon_stiffness)
        self.tendon_damping        = float(tendon_damping)
        self.tendon_motor_inertia  = float(tendon_motor_inertia)
        # Set by build_plant()
        self.builder     = None
        self.plant       = None
        ...
        self.tendon_sys  = None # set if tendon_stiffness > 0
...
class IKPDSimulation(Simulation):
    ...
    def __init__(self, manip_config, meshcat=None,
        ...
        super().__init__(manip_config, meshcat,
                         ...
                         tendon_motor_inertia=tendon_motor_inertia,
                         **kwargs)
    ...
    def connect_and_build(self):
        ...
        # Tendon spring (SEA): real spring F_s = k_s*(r_p*(θ_m - q₂)) + b_s*(r_p*(θ̇_m - q̇₂))
        if self.tendon_stiffness > 0:
            self.tendon_sys = self.builder.AddSystem(
                TendonSpringCompliance(
                    self.plant, self.manipulator,
                    k_s=self.tendon_stiffness,
                    b_s=self.tendon_damping,
                    I_m=self.tendon_motor_inertia,
                )
            )
            self.tendon_sys.set_name("Tendon_Spring_SEA")
            self.builder.Connect(
                self.ik_system.GetOutputPort("actuation"),
                self.tendon_sys.GetInputPort("torque_cmd"),
            )
            self.builder.Connect(
                self.plant.get_state_output_port(),
                self.tendon_sys.GetInputPort("plant_state"),
            )
            self.builder.Connect(
                self.tendon_sys.GetOutputPort("torque_out"),
                self.plant.get_actuation_input_port(),
            )
            print(colored(
                f"  ✓ Tendon spring SEA: k_s={self.tendon_stiffness:.1f} N/m"
                f"  b_s={self.tendon_damping:.2f} N·s/m"
                f"  I_m={self.tendon_motor_inertia:.2e} kg·m²"
                f"  (joint 2 real spring — weak k_s → laggy/oscillatory J2)",
                "yellow",
            ))
        else:
            self.builder.Connect(
        ...
        # Loggers — log the actual torque arriving at the plant
        _act_src = (self.tendon_sys.GetOutputPort("torque_out")
                    if self.tendon_sys is not None
                    else self.ik_system.GetOutputPort("actuation"))
        ...
        self.loggers['actuation']= LogVectorOutput(_act_src,                                          self.builder)
    ...
    def initialize(self, home_override=None):
        ...
        # Initialise SEA motor angle = q2_init to avoid initial spring jerk
        if self.tendon_sys is not None:
            tendon_ctx = self.tendon_sys.GetMyMutableContextFromRoot(
                self.simulator.get_mutable_context()
            )
            tendon_ctx.get_mutable_continuous_state_vector().SetAtIndex(0, q0[1])  # θ_m = q2
            tendon_ctx.get_mutable_continuous_state_vector().SetAtIndex(1, 0.0)   # θ̇_m = 0
    ...
    def run(self, traj_shape: str = 'trajectory'):
        ...
        print(colored(
            ...
            + (f"\n   Tendon spring SEA: k_s={self.tendon_stiffness:.1f} N/m  b_s={self.tendon_damping:.2f} N·s/m  I_m={self.tendon_motor_inertia:.2e} kg·m²  (J2 real spring)"
               if self.tendon_stiffness > 0 else "")
...
class ComputedTorqueSimulation(IKPDSimulation):
    ...
    def __init__(self, manip_config, meshcat=None, Kp=400.0, Kd=40.0, tau_max=10.0, **kwargs):
        super().__init__(manip_config, meshcat, **kwargs)
...
_SIM_REGISTRY: dict = {
    'pd-control': (
        IKPDSimulation,
        lambda args: dict(
            tendon_stiffness=args.tendon_stiffness,
            tendon_damping=args.tendon_damping,
            tendon_motor_inertia=args.tendon_motor_inertia,
        ),
