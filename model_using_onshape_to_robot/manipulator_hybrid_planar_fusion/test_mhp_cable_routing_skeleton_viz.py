...
try:
    ...
    from pydrake.geometry import (
        Cylinder as _DrakeCylinder,
        Sphere   as _DrakeSphere,
        Mesh     as _DrakeMesh,
        Rgba     as _DrakeRgba,
    )
...
def _cylinder_transform_drake(p1: np.ndarray, p2: np.ndarray) -> np.ndarray:
    """4×4 transform for Drake's Z-aligned Cylinder geometry on segment p1→p2."""
    mid = (p1 + p2) / 2.0
    direction = p2 - p1
...
def _cylinder_transform(p1: np.ndarray, p2: np.ndarray) -> np.ndarray:
    ...
    Used for raw meshcat's ``Cylinder`` geometry (Y-axis aligned).
    """
    mid = (p1 + p2) / 2.0
    direction = p2 - p1
    length = np.linalg.norm(direction)
...
# ─── Drake Meshcat scene functions ──────────────────────────────────────────

def _draw_skeleton_drake(
    mc, kin: "MHPKinematics",
    lower: CableRouteConfig,
    upper: CableRouteConfig,
) -> None:
    """Draw arm skeleton + cable tubes on a Drake Meshcat server.

    Clears the previous frame first, then redraws everything from the
    current :class:`MHPKinematics` state.
    """
    mc.Delete("skeleton")
    mc.Delete("cables")
    ...
    bone_rgba  = _DrakeRgba(0.13, 0.40, 0.67, 0.20)
    joint_rgba = _DrakeRgba(1.00, 0.53, 0.00, 0.90)

    for i, (p1, p2) in enumerate([(base, J1), (J1, J2), (J2, EE)]):
        length = float(np.linalg.norm(p2 - p1))
        if length < 1e-6:
            continue
        T44 = _cylinder_transform_drake(p1, p2)
        X   = RigidTransform(RotationMatrix(T44[:3, :3]), T44[:3, 3])
        mc.SetObject(f"skeleton/bone_{i}", _DrakeCylinder(0.009, length), bone_rgba)
        mc.SetTransform(f"skeleton/bone_{i}", X)

    for i, pt in enumerate([J1, J2]):
        mc.SetObject(f"skeleton/joint_{i}", _DrakeSphere(0.014), joint_rgba)
        mc.SetTransform(f"skeleton/joint_{i}", RigidTransform(pt))

    # ── Physical components: OBJ meshes with sphere fallback ─────────────────
    import os as _os
    _script_dir  = _os.path.dirname(_os.path.abspath(__file__))
    _assets_dir  = _os.path.join(_script_dir, 'assets')
    ...
    for route in (lower, upper):
        for comp in route.physical:
            ...
            # World-frame position + rotation, respecting visual_pos_in_link/rpy
            if comp.visual_pos_in_link is not None and comp.role != "spool":
                pw      = kin.to_world(comp.visual_pos_in_link, comp.link)
                R_local = _rpy_to_mat(*comp.visual_rpy) if comp.visual_rpy is not None else np.eye(3)
                R_world = kin.link_frame_rotation(comp.link) @ R_local
            else:
                pw      = kin.to_world(comp.pos_in_link, comp.link)
                R_world = kin.link_frame_rotation(comp.link)
            X = RigidTransform(RotationMatrix(R_world), pw)

            obj_path = _os.path.join(_assets_dir, comp.obj_filename)
            path     = f"skeleton/comp_{comp.name}"
            if _os.path.exists(obj_path):
                try:
                    mc.SetObject(path, _DrakeMesh(obj_path, scale=1.0))
                    mc.SetTransform(path, X)
                    continue
                except Exception as _e:
                    print(f"  ⚠ OBJ load failed for {comp.obj_filename}: {_e}")

            # Fallback: coloured sphere
            r = comp.diameter_mm * 0.5e-3
            if r < 1e-6:
                continue
            hex_str   = comp.color.lstrip('#')
            comp_rgba = _DrakeRgba(
                int(hex_str[0:2], 16) / 255.0,
                int(hex_str[2:4], 16) / 255.0,
                int(hex_str[4:6], 16) / 255.0,
                0.50,
            )
            mc.SetObject(path, _DrakeSphere(r), comp_rgba)
            mc.SetTransform(path, X)
    ...
    for route in (lower, upper):
        ...
        for piece in cp.pieces:
            ...
            for k in range(len(pts) - 1):
                p1 = pts[k].astype(float)
                p2 = pts[k + 1].astype(float)
                ...
                mc.SetObject(path, _DrakeCylinder(CABLE_RADIUS, seg_len), cable_rgba)
                mc.SetTransform(path, X)
                seg_idx += 1
...
def build_meshcat_scene(
    ...
    """Start a Meshcat server and return a reusable handle dict.

    Uses Drake's ``StartMeshcat`` when pydrake is available (preferred),
    otherwise falls back to the raw ``meshcat`` library.
    ...
    if DRAKE_AVAILABLE:
        mc = _DrakeStartMeshcat()
        print(f"\n✓ Meshcat (Drake): {mc.web_url()}")
        return {"mode": "drake", "mc": mc}
    ...
    if MESHCAT_AVAILABLE:
        ...
        print(f"\n✓ Meshcat (raw): {url}")
        return {"mode": "meshcat", "vis": vis}

    print("⚠️  Neither pydrake nor meshcat available.")
    return None
...
def update_meshcat_pose(
    ...
    """Re-pose the skeleton to (q1, q2) [rad] and redraw skeleton + cables."""
    if scene is None:
        return
    ...
    if scene["mode"] == "drake":
        _draw_skeleton_drake(scene["mc"], kin, lower, upper)
    else:
        ...
        J2   = kin.J2
        EE   = kin.to_world(np.array([0.2, 0., 0.]), "lower_arm")
        bone_mat  = g.MeshPhongMaterial(color=0x2266aa, opacity=0.15)
        joint_mat = g.MeshPhongMaterial(color=0xff8800, opacity=0.9)
        for i, (p1, p2) in enumerate([(base, J1), (J1, J2), (J2, EE)]):
            length = np.linalg.norm(p2 - p1)
            vis[f"arm_{i}"].set_object(g.Cylinder(length, 0.008), bone_mat)
        ...
        for route in (lower, upper):
            ...
            for piece in cp.pieces:
                for k in range(len(piece) - 1):
                    p1, p2 = piece[k].astype(float), piece[k + 1].astype(float)
                    seg_len = np.linalg.norm(p2 - p1)
                    if seg_len < 1e-6:
                        continue
                    vis[f"cable_{route.name}/seg_{seg_idx}"].set_object(
                        g.Cylinder(seg_len, CABLE_RADIUS), cable_mat)
...
def visualize_cable_routing_meshcat(
    ...
    """Thin wrapper: build scene + draw initial pose.  Returns scene handle."""
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(
        description="MHP cable routing skeleton visualisation — 3D + XY top view + Drake Meshcat"
    )
    ...
    ap.add_argument("--show", action='store_true',
                    help="Show interactive plot window alongside Meshcat")
    ap.add_argument("--no-matplotlib", dest="no_matplotlib", action='store_true',
                    help="Skip matplotlib figure; go straight to Meshcat interactive loop")
    args = ap.parse_args()
    ...
    if not args.no_matplotlib:
        ...
        if args.save:
            ...
            print("   • Close the figure window to continue to the Meshcat loop\n")
            plt.show()

    # ── Meshcat scene — built once, then re-posed interactively ───────────────
    scene = build_meshcat_scene(lower_cable, upper_cable)
    if scene is None:
        sys.exit(0)

    update_meshcat_pose(scene, lower_cable, upper_cable, q1_rad, q2_rad)
    backend_url = (scene["mc"].web_url() if scene["mode"] == "drake"
                   else scene["vis"].url())
    print(f"\n  Open browser → {backend_url}")
    print(colored(
        "\nEnter joint angles in degrees  (e.g.  30  -15)  →  skeleton + cables update.",
        "yellow"))
        ...
        try:
            while True:
                ...
                try:
                    ...
                    continue
                update_meshcat_pose(scene, lower_cable, upper_cable,
                                    np.deg2rad(q1_deg), np.deg2rad(q2_deg))
                kin2 = MHPKinematics(np.deg2rad(q1_deg), np.deg2rad(q2_deg))
                ee = kin2.to_world(np.array([0.2, 0., 0.]), "lower_arm")
                print(colored(
