...
\begin{lstlisting}[caption={Mandatory import sequence --- \texttt{rl/script\_cup\_manipulator\_computed\_torque\_isaac\_sim.py}}]
...
# FIRST import --- must precede everything else
...
\begin{lstlisting}[caption={Engine-agnostic CT controller --- \texttt{rl/computed\_torque\_controller.py}}]
...
\begin{lstlisting}[caption={IK-to-joint-space pipeline --- \texttt{rl/computed\_torque\_controller.py}}]
...
\begin{lstlisting}[caption={Looping cubic spline --- \texttt{rl/trajectory.py}}]
...
\begin{lstlisting}[caption={Preamble construction --- \texttt{rl/trajectory.py}}]
...
\begin{lstlisting}[caption={World and robot setup --- \texttt{rl/script\_cup\_manipulator\_computed\_torque\_isaac\_sim.py}}]
...
\begin{lstlisting}[caption={Cable visualization wrapper --- \texttt{rl/cable\_viz.py}}]
...
# Headless mode (no rendering -- for benchmarking)
python rl/script_cup_manipulator_computed_torque_isaac_sim.py \
    --render headless --duration 10.0
...
\begin{itemize}[leftmargin=2em, topsep=2pt, itemsep=1pt]
    \item \texttt{rl/script\_cup\_manipulator\_computed\_torque\_isaac\_sim.py} --- Main Isaac Sim simulation script
    \item \texttt{robots/cup\_manipulator\_tendon\_isaac.py} --- Isaac Sim robot wrapper (URDF$\to$USD, ArticulationView)
    \item \texttt{rl/computed\_torque\_controller.py} --- Pure-NumPy CT controller (shared with PyDrake)
    \item \texttt{rl/trajectory.py} --- SciPy trajectory classes (Rect, Circle, Line, Preamble)
    \item \texttt{rl/cable\_viz.py} --- Cable visualization via DrakeCablePlant + USD prims
    \item \texttt{cable.py} --- DrakeCablePlant for headless cable FK
\end{itemize}
\end{small}
