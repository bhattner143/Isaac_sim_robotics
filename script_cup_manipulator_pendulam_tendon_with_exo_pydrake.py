...
# ─── _PreambleSrc ───────────────────────────────────────────────────────────
# PURPOSE:
#   Time-multiplexed trajectory source that wraps a main periodic trajectory
#   with an optional move-to-start preamble.  The preamble smoothly moves the
#   end-effector from its current (rest) position to the first waypoint of the
#   main trajectory before the main loop begins.
#
# USED IN CURRENT IMPLEMENTATION: YES — always instantiated (3 instances).
#   • ee_src  : outputs [x, y] EE position reference
#   • vel_src : outputs [ẋ, ẏ] EE velocity reference
#   • acc_src : outputs [ẍ, ÿ] EE acceleration reference
#   All three feed into the ComputedTorqueController (CT).
#
# SIGNAL FLOW:
#   _PreambleSrc → CT.desired_ee_pos / CT.ee_vel_ref / CT.ee_acc_ref
#
# BEHAVIOUR:
#   • t < move_duration  → evaluates the preamble trajectory (CubicHermite
#     interpolation from rest pose to the first waypoint).
#   • t ≥ move_duration  → evaluates the main periodic trajectory, wrapping
#     the phase so laps repeat correctly.
#   If move_duration == 0, the preamble path is None and only the main
#   trajectory is used from t = 0.

class _PreambleSrc(LeafSystem):
    """Trajectory source with optional move-to-start preamble."""
    def __init__(self, mv, md, main_traj, period):
        super().__init__()
        self._mv     = mv
        self._md     = float(md)
...
# ════════════════════════════════════════════════════════════════════════════
# Trajectory builders  (identical to the base SEA script)
# ════════════════════════════════════════════════════════════════════════════
...
def _clamp_to_reach(ee_x, ee_y, r_max, r_min):
    _r = np.hypot(ee_x, ee_y)
    _far = _r > r_max
    if _far.any():
        ee_x[_far] *= r_max * 0.97 / _r[_far]
...
def _build_circle_trajectory(manip, plant, args):
    L1, L2 = manip.ik.get_link_lengths(plant)
    r_max  = L1 + L2
    r_min  = abs(L1 - L2)
...
def _build_figure8_trajectory(manip, plant, args):
    L1, L2 = manip.ik.get_link_lengths(plant)
    r_max  = L1 + L2
    r_min  = abs(L1 - L2)
...
def _build_line_trajectory(manip, plant, args):
    """Back-and-forth along y at fixed x = mean(x_range)."""
    L1, L2 = manip.ik.get_link_lengths(plant)
    r_max  = L1 + L2
    r_min  = abs(L1 - L2)
...
def _build_trajectory(manip, plant, args):
    kind = getattr(args, "traj_type", "rect")
    if kind == "circle":
        return _build_circle_trajectory(manip, plant, args)
    if kind == "figure8":
        return _build_figure8_trajectory(manip, plant, args)
    if kind == "line":
        return _build_line_trajectory(manip, plant, args)
    return _build_rect_trajectory(manip, plant, args)
...
def run_simulation(meshcat) -> dict:
    ...
    traj, traj_vel, traj_acc, ee_x_tgt, ee_y_tgt = \
        _build_trajectory(manipulator, plant, args)
