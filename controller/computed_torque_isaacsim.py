"""
controller/computed_torque_isaacsim.py
──────────────────────────────────────
Computed-torque (inverse-dynamics) controller — pure NumPy, no Drake.

This is the Isaac Sim counterpart of the Drake-based controllers in
``controller/controller.py``.  It uses only NumPy so it can run with
Isaac Sim's ``ArticulationView`` (or any source of M(q) and h(q,q̇))
without requiring a PyDrake installation.
