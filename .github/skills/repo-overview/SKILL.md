...
|----------|----------------------|
| Rigid/inextensible cable tendon | `SEACableActuator` with `k_s`, `b_c` spring |
| Motor torque → cable tension → joint | Software spring model `F = k_s·δ + b_c·δ̇` |
| CT → MIT `tau_ff` (or tension inner loop if load cells added) | CT → SEA → plant |
| Do **not** add `SEACableActuator` or high `k_s` stiff-spring hacks for hardware/MHP sim | SEA tuning applies to digital-twin scripts only |
