...
parser.add_argument('--target-x', type=float, default=-0.8, help='Target X position [m]')
...
def run_finite_horizon_lqr_cart_pend_only(
    ...
    Args:
        ...
        initial_q: Initial manipulator joint angles [q2, q1] (Drake order)
    """
    # Linearize system
    ...
    if manipulator is not None:
        # CRITICAL: Drake expects [q2, q1] order. Setting q1=-10°, q2=20°
        if initial_q is None:
            initial_q = np.array([np.deg2rad(20.0), np.deg2rad(-10.0)])
        
        plant.SetPositions(plant_context, manipulator.model_instance, initial_q)
...
def run_lqr_manip_ee_traj_track(
    ...
    manip_controller = builder.AddSystem(
        ComputedTorqueEEController(manipulator, plant, Kp=800.0, Kd=80.0, tau_max=200.0)
    )
    ...
    builder.Connect(
        plant.get_state_output_port(manipulator.model_instance),  # [q1, q2, q̇1, q̇2] manipulator state
        manip_controller.get_input_port(1)                        # → controller manipulator state
    )
    
    # Connect controller output to manipulator actuation
    builder.Connect(
        manip_controller.get_output_port(),                       # [τ1, τ2] joint torques
    ...
    # Add logger for manipulator state
    manip_state_logger = builder.AddSystem(VectorLogSink(4))
    manip_state_logger.set_name("manip_state_logger")
    builder.Connect(plant.get_state_output_port(manipulator.model_instance), manip_state_logger.get_input_port())
    ...
    # CRITICAL: Drake expects [q2, q1] order
    manipulator_positions = np.array([q_init[1], q_init[0]])
    plant.SetPositions(plant_context, manipulator.model_instance, manipulator_positions)
    plant.SetVelocities(plant_context, manipulator.model_instance, np.zeros(2))
    ...
    # ========================================================================
    print(colored("\n📸 Visualizing configured initial state...", "cyan"))
    print(colored(f"  - Manipulator: q1={np.rad2deg(manipulator_positions[1]):.1f}°, q2={np.rad2deg(manipulator_positions[0]):.1f}° (will track cart)", "cyan"))
    print(colored(f"  - Cart: ({cart_x:.3f}, {cart_y:.3f}) m", "cyan"))
    print(colored(f"  - Pendulum: α=0°, β=0° (hanging)", "cyan"))
    diagram.ForcedPublish(context)
    ...
    force_log = force_logger.FindLog(context)
    impedance_log = impedance_logger.FindLog(context)
    manip_state_log = manip_state_logger.FindLog(context)
    ...
    force_data = force_log.data()
    impedance_data = impedance_log.data()
    manip_state_data = manip_state_log.data()  # [q2, q1, q̇2, q̇1] in Drake order
    cart_traj_data = cart_traj_log.data()  # [x, y, ẋ, ẏ, ẍ, ÿ] sent to controller
    manip_torque_data = manip_torque_log.data()  # [τ1, τ2]
    ...
    for i in range(len(t)):
        ...
        # Set manipulator state at this timestep from logged data
        plant.SetPositions(temp_context, manipulator.model_instance, manip_state_data[0:2, i])
        plant.SetVelocities(temp_context, manipulator.model_instance, manip_state_data[2:4, i])
    ...
    # Manipulator joint angles from logged data (Drake order: [q2, q1])
    q1_deg = np.rad2deg(manip_state_data[1, :])  # manip_state_data[1] is q1 in Drake order
    q2_deg = np.rad2deg(manip_state_data[0, :])  # manip_state_data[0] is q2 in Drake order
    ax17.plot(t, q1_deg, 'b-', linewidth=2, label='q1 (link1)')
    ax17.plot(t, q2_deg, 'r-', linewidth=2, label='q2 (link2)')
    ax17.set_xlabel('Time [s]')
    ax17.set_ylabel('Joint Angles [deg]')
    ax17.legend()
    ax17.grid(True)
    ax17.set_title('Manipulator Joint Angles (Tracking Cart)')
    
    ax18 = fig.add_subplot(gs[4, 1])
    # Joint velocities from logged data
    q1_dot_deg = np.rad2deg(manip_state_data[3, :])  # manip_state_data[3] is q̇1
    q2_dot_deg = np.rad2deg(manip_state_data[2, :])  # manip_state_data[2] is q̇2
    ax18.plot(t, q1_dot_deg, 'b-', linewidth=2, label='q̇1')
    ax18.plot(t, q2_dot_deg, 'r-', linewidth=2, label='q̇2')
    ax18.set_xlabel('Time [s]')
    ax18.set_ylabel('Joint Velocities [deg/s]')
    ax18.legend()
    ax18.grid(True)
    ax18.set_title('Manipulator Joint Velocities')
...
def main():
    ...
    manipulator.weld_base_to_world(plant)  # Fix base to world origin
    ...
    # To set q1=-10°, q2=20°, we pass [20°, -10°]
    initial_q = np.array([np.deg2rad(80.0), np.deg2rad(-40.0)])
    
    # Calculate EE position at configured joint angles
    temp_context = plant.CreateDefaultContext()
    plant.SetPositions(temp_context, manipulator.model_instance, initial_q)
    ee_pos_init = manipulator.CalcPosition(plant, temp_context)
    
    # Define cart initial position at manipulator EE
    cart_init_pos = np.array([ee_pos_init[0], ee_pos_init[1], 0.0, 0.0])  # [x, y, α, β]
    
    # Print configuration summary
    print(colored(f"\n📄 Initial Configuration:", "cyan"))
    print(colored(f"  - Manipulator: q1={np.rad2deg(initial_q[1]):.1f}°, q2={np.rad2deg(initial_q[0]):.1f}°", "cyan"))
    print(colored(f"  - Manipulator EE: ({ee_pos_init[0]:.3f}, {ee_pos_init[1]:.3f}, {ee_pos_init[2]:.3f}) m", "cyan"))
    ...
    if args.mode == 'scene-viz':
        # Add systems to builder for scene visualization
